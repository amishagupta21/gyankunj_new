...
const CreateMasterRoutine = ({ isOpen, handleClose, selectedData = {} }) => {
  ...
  useEffect(() => {
    if (selectedData && selectedData.subject_id) {
      setIsEditMode(true);
      setValue("subject_id", selectedData.subject_id);
      setValue("teacher_id", selectedData.teacher_id);
     // const dateTimeString = `${currentDate}T${timeString}:00.000Z`;
      setValue("start_time", dayjs(selectedData.start_time).format('HH:mm'));
      setValue("end_time", dayjs(selectedData.end_time));
      setStartTime(dayjs(selectedData.start_time));
    }
